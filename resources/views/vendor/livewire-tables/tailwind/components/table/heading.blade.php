...
<th
    {{ $attributes->merge(['class' => 'px-3 py-2 md:px-6 md:py-3 bg-indigo-500'])->only('class') }}
>
    @unless ($sortable)
        <span class="block text-left text-xs leading-4 font-medium text-gray-50 uppercase tracking-wider">
            {{ $text ?? $slot }}
        </span>
    @else
        <button
            ...
            class="flex items-center space-x-1 text-left text-xs leading-4 font-medium text-gray-50 uppercase tracking-wider group focus:outline-none focus:underline"
        >
            <span>{{ $text ?? $slot }}</span>
